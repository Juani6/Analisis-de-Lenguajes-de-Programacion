...
update var n (s,str) = (M.insert var n s, str) 

-- Agrega una traza dada al estado
-- Completar la definición
addTrace :: String -> State -> State
addTrace str (s, str') = (s, str' ++ str)
...
stepComm (Let v e) s              = case evalExp e s of
                                         (Right (e0 :!: s')) -> let t  = update v e0 s'
                                                                    t' = addTrace (toStr v e0) t 
                                                                in Right $ Skip :!: t'
                                         Left err -> Left err 
...
toStr :: Variable -> Int -> String 
toStr v e0 = "(" ++ "Let "  ++ v ++ " " ++ (show e0) ++ ")"++ " "
